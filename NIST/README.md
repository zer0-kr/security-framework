...
```
NIST 사이버보안/프라이버시 출판물
├── FIPS    — 연방 정보처리 표준 (의무 준수)
├── SP 800  — 특별 간행물: 컴퓨터/정보 보안 (권고)
├── SP 1800 — 특별 간행물: 사이버보안 실무 가이드 (구현 예시)
├── IR      — 내부 보고서 (연구/워크숍/분석)
├── CSWP    — 사이버보안 백서 (신규 주제/의견)
└── 프레임워크
    ├── CSF  — Cybersecurity Framework (사이버보안 프레임워크)
    └── PF   — Privacy Framework (프라이버시 프레임워크)
```
...
└──────────────────────┬──────────────────────────────────────┘
                       │ 매핑
┌──────────────────────▼──────────────────────────────────────┐
│              SP 800-53 (보안/프라이버시 컨트롤)                │
│                "어떤 통제를 적용할 것인가"                      │
│          20 Families, 1,196 Controls (base + enhancements)   │
└──────────┬───────────────────────────┬──────────────────────┘
           │                           │
     ┌─────▼─────┐              ┌──────▼──────┐
     │ SP 800-53A │              │ SP 800-53B  │
     │ 평가 절차   │              │ 기준선(베이스라인) │
     └───────────┘              └─────────────┘
           │
     ┌─────▼──────────────────────────────────────────────────┐
     │         SP 800 시리즈 (주제별 상세 가이드)                 │
     │  800-37(RMF), 800-171(CUI), 800-61(침해대응) ...         │
     └──────────────────────┬─────────────────────────────────┘
                            │ 구현 예시
     ┌──────────────────────▼─────────────────────────────────┐
     │            SP 1800 시리즈 (실무 가이드)                    │
...
| 항목 | 내용 |
|------|------|
| **성격** | 연방 표준 — 미국 연방기관은 **의무 준수** |
| **법적 근거** | FISMA에 의해 NIST에 표준 개발 권한 부여 |
...
| FIPS 200 | Minimum Security Requirements for Federal Information and Information Systems | 연방 정보시스템 최소 보안 요구사항 |
| FIPS 201-3 | Personal Identity Verification (PIV) | 연방 직원/계약자 신원확인 표준 |
| FIPS 202 | SHA-3 Standard | SHA-3 해시 함수 표준 |
| FIPS 203 | Module-Lattice-Based Key-Encapsulation Mechanism (ML-KEM) | 양자내성 키 캡슐화 표준 |
| FIPS 204 | Module-Lattice-Based Digital Signature Algorithm (ML-DSA) | 양자내성 디지털 서명 표준 |
| FIPS 205 | Stateless Hash-Based Digital Signature Algorithm (SLH-DSA) | 양자내성 해시 기반 서명 표준 |
...
### 2. SP 800 시리즈 (Special Publications — Computer Security)

| 항목 | 내용 |
|------|------|
| **성격** | 기술 가이드라인 및 권고사항 |
| **범위** | 컴퓨터/정보 보안 전반 |
| **문서 수** | 약 220건 이상 (Final + Draft) |
| **URL** | https://csrc.nist.gov/publications/sp800 |

NIST 사이버보안의 핵심 시리즈입니다. 번호는 800-12부터 800-236까지 분포하며, 주요 문서를 주제별로 분류하면:
...
| 문서 | 제목 | 핵심 내용 |
|------|------|----------|
| SP 800-53 Rev. 5 | Security and Privacy Controls | **보안/프라이버시 컨트롤 카탈로그.** 20개 패밀리, 1,000+ 컨트롤. NIST 체계의 핵심 |
...
### 3. SP 1800 시리즈 (Cybersecurity Practice Guides)
...
| **문서 수** | 약 41건 |
| **URL** | https://csrc.nist.gov/publications/sp1800 |

SP 800이 "무엇을, 왜"를 설명한다면, SP 1800은 **"실제로 어떻게 구현하는가"** 를 보여줍니다. 각 문서는 보통 3개 파트로 구성됩니다:
...
### 4. NIST IR (Internal/Interagency Reports)

| 항목 | 내용 |
|------|------|
| **성격** | 내부 보고서 — 연구 결과, 워크숍 결과, 분석 보고서 |
| **문서 수** | 약 326건 이상 |
| **URL** | https://csrc.nist.gov/publications/ir |

SP 시리즈가 되기 전 단계의 연구나, 특정 주제에 대한 심층 분석을 담습니다.
...
### 5. CSWP (Cybersecurity White Papers)

| 항목 | 내용 |
|------|------|
| **성격** | 백서 — 신규 주제, 의견 제시, 프레임워크 문서 |
| **문서 수** | 약 55건 |
| **URL** | https://csrc.nist.gov/publications/white-papers |
...
| Function | 코드 | 설명 |
|----------|------|------|
| **Govern** | GV | 조직의 사이버보안 리스크 관리 전략, 기대치, 정책을 수립하고 모니터링. **CSF 2.0에서 신설** | 6 Categories |
| **Identify** | ID | 현재의 사이버보안 리스크를 이해 — 자산, 취약점, 위협 식별 | 3 Categories |
| **Protect** | PR | 보안 위험을 관리하기 위한 보호조치 적용 — 접근제어, 교육, 데이터 보안 | 5 Categories |
| **Detect** | DE | 사이버보안 공격과 침해를 탐지 — 지속적 모니터링, 이상 분석 | 2 Categories |
| **Respond** | RS | 탐지된 사이버보안 사고에 대응 — 사고 관리, 분석, 완화, 보고 | 4 Categories |
| **Recover** | RC | 사이버보안 사고로 영향받은 자산과 운영을 복구 | 2 Categories |
...
|------|------|
| **발행일** | 2020년 1월 16일 |
| **차기 버전** | PF 1.1 (Initial Public Draft, 2025년 4월) |
...
## 기계 판독 가능 데이터 (Machine-Readable Data)
